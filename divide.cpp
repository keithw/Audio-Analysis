...
// arguments: dividend_filename divisor_filename quotient_filename
...
int main( int argc, char **argv )
{
  ...
  if ( scmanal->get_type( 0 ) == FFT_DOMAIN ) {
    ...
    for ( uint i = 1; i < (length+1) / 2; i++ ) {
      ...
      double denom = b_real * b_real + b_imag * b_imag;

      if ( denom == 0 ) {
	output[ i ] = NAN;
	output[ length - i ] = NAN;
      } else {
	output[ i ]          = (a_real * b_real + a_imag * b_imag) / denom;
	output[ length - i ] = (a_imag * b_real - a_real * b_imag) / denom;
      }
    }
